...
MODEL_PATH = "GLM-4.6V-Flash" 
# MODEL_PATH = "/path/to/your/local/glm-model" 

LORA_PATH = os.path.join(PROJECT_ROOT, "LLaMA-Factory/saves/glm4_6v_flash/lora/sft") 
IS_LORA = True 


BASE_DIR = os.path.join(PROJECT_ROOT, "simdata_benchmark")
JSON_FILE = "pure_sim_test.json"
OUTPUT_FILE = "sim_testset_glm_4_6v_lora.json" if IS_LORA else "sim_testset_glm4_6v.json"
...
def parse_ground_truth(answer_text, q_type):
    ...
    if q_type == "Multiple_Choice":
        ...
        if "true" in text_lower: return "True"
        if "false" in text_lower: return "False"
        if "yes" in text_lower: return "True"
        if "no" in text_lower: return "True"
        return text
...
def main():
    ...
    for entry in tqdm(dataset):
        ...
        try:
            image_obj = Image.open(image_full_path).convert("RGB")
        except Exception as e:
            print(f"Corrupted image: {image_full_path}")
            continue
